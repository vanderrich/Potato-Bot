...
module.exports = {
    ...
    async execute(interaction: CommandInteraction | ContextMenuInteraction, client: any, footers: Array<string>) {
        ...
        const embed = new MessageEmbed()
            .setAuthor({ name: await client.getLocale(interaction.user.id, "commands.currency.inv.title"), iconURL: user.displayAvatarURL() })
            .setColor("RANDOM")
            .setFooter({ text: footers[Math.floor(Math.random() * footers.length)] })
        const invPure = await client.eco.getUserItems({ user });
        if (!invPure) {
            embed.setDescription(await client.getLocale(interaction.user.id, "commands.currency.inv.noItems"));
            ...
            do {
                ...
                if (items.length) {
                    const embed = new MessageEmbed();
                    embed.setAuthor({ name: await client.getLocale(interaction.user.id, "commands.currency.inv.title", user.username), iconURL: user.displayAvatarURL() })
                    embed.setDescription(`${items.join('\n')}${inv.length > pageEnd
                        ? `\n${await client.getLocale(interaction.user.id, "commands.currency.inv.moreItems", pageEnd - inv.length)}`
                        : ''
                        } `);
                    ...
                    if (page === 1) {
                        const embed = new MessageEmbed()
                            .setAuthor({ name: await client.getLocale(interaction.user.id, "commands.currency.inv.title", user.username), iconURL: user.displayAvatarURL() })
                            .setColor('RANDOM')
                            .setDescription(await client.getLocale(interaction.user.id, "commands.currency.inv.noItems"))
                            .setFooter({ text: footers[Math.floor(Math.random() * footers.length)] });
                        return interaction.reply({ embeds: [embed] });
                    }
                    if (page === 2) {
