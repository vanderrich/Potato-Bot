...
module.exports = {
    ...
    async execute(interaction: CommandInteraction, client: any, footers: string[]) {
        ...
        const categories = await client.getLocale(interaction.user.id, "commands.info.help.categories");
        let category = interaction.options.getString("category")
        if (category) {
            category = category.charAt(0).toUpperCase() + category?.slice(1)
            if (!categories.includes(category)) {
                return interaction.reply(await client.getLocale(interaction.user.id, "commands.info.help.noCategory"));
            }
            ...
            if (!commandObject) {
                return interaction.reply(await client.getLocale(interaction.user.id, "commands.info.help.noCommand"))
            }
            const messageEmbed = new MessageEmbed()
                .setColor("RANDOM")
        ...
        for (let category in categories) {
            ...
            a.push({ name: categories[category], value: await client.getLocale(interaction.user.id, "commands.info.help.commandsInCategory", value) })
        }
        const messageEmbed = new MessageEmbed()
            .setColor('RANDOM')
            .setTitle(await client.getLocale(interaction.user.id, "commands.info.help.embedTitle"))
            .setDescription(await client.getLocale(interaction.user.id, "commands.info.help.embedDesc"))
            .addFields(...a)
            .setFooter({ text: footers[Math.floor(Math.random() * footers.length)] })
