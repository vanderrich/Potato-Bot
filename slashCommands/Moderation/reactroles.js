...
module.exports = {
    ...
    execute(interaction, client, Discord, footers) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!channel)
                return interaction.reply("Please specify channel!");
            if (!(channel instanceof Discord.TextChannel))
                return interaction.reply("Please specify a text channel!");
            let options = [];
            let reactionRoles = [];
            let reactions = [];
            if (!interaction.guild)
                return interaction.reply("You can't use this command in a DM!");
            if (!((_a = interaction.guild.me) === null || _a === void 0 ? void 0 : _a.roles.highest.position))
            ...
            for (let i = 1; i <= 25; i++) {
                ...
                if (option && emoji && role) {
                    if (role.position < interaction.guild.me.roles.highest.position) {
                        if (emoji.match(/<a?:[a-zA-Z0-9_]+:[0-9]+>/g))
