...
module.exports = {
  ...
  async execute(msg, args, cmd, client, Discord, footers) {
    ...
    const embed = new Discord.MessageEmbed()
      ...
      .setFooter({ text: 'to stop the game for everyone, type \`stopgame\`', iconURL: message.author.avatarURL({ dynamic: true }) })
    ...
    async function play(players) {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        message.reply(word)
        message.channel.awaitMessages({ time: 30000, errors: ['time'] }).then((m) => {
          if (m.content === 'stop') {
            message.reply(`${m.author} stopped the game!`)
            collector.stop()
            return
          }
